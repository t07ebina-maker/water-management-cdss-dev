...
function calcEGFR(age, sex, cre) {
  ...
  if (age < 18) return null; // 日本人式は成人（18歳以上）のみ適用
...
function validate() {
  ...
  let s1Errors = 0, s1Empty = 0;

  // Age
  const ageR = checkRange('age', 0, 120, '年齢');
  if (!ageR.ok) { if (ageR.empty) s1Empty++; else s1Errors++; }

  // Sex
  if (!vs('sex')) { s1Empty++; } else { setField('sex', 'ok', ''); }
  ...
  } else if ((!sbpR.ok && !sbpR.empty) || (!dbpR.ok && !dbpR.empty)) {
    // 範囲外の入力あり → エラー
    s2Errors++;
  } else {
    // 血圧は必須 — 片方でも両方でも未入力なら未入力計上（両方空欄のすり抜け防止）
    s2Empty++;
  }

  // HR
  const hrR = checkRange('hr', 30, 200, '心拍数');
  if (!hrR.ok) { if (hrR.empty) s2Empty++; else s2Errors++; }
  ...
  if (!rrR.ok) { if (rrR.empty) s2Empty++; else s2Errors++; }

  // SpO2
  const spo2R = checkRange('spo2', 70, 100, 'SpO₂');
  if (!spo2R.ok) { if (spo2R.empty) s2Empty++; else s2Errors++; }
  if (spo2R.ok) {
  ...
  if (!tempR.ok) { if (tempR.empty) s2Empty++; else s2Errors++; }
  if (tempR.ok && tempR.val > 37.0) {
    // 発熱補正は37℃超で連続適用されるため、通知も37℃超で常に表示（補正と通知の整合）
    alerts.info.push({ title: '発熱', msg: `体温 ${tempR.val}℃。維持輸液量に発熱補正が必要です（37℃超1℃ごと +15%）。` });
  }
  ...
  if (urinePerDay !== null) {
    ...
    // 尿量は任意 — 未入力でも第2段階へ進めるが、乏尿評価が行えないことを明示する
    alerts.info.push({ title: '尿量未評価', msg: '尿量が未入力です。乏尿・無尿の判定を行っていません。可能なら時間尿または1日尿量を確認してください。' });
    setField(urineMode === 'h' ? 'urineH' : 'urineD', '', '');
  }

  setSection('s2-status', s2Errors, s2Empty);
  ...
  } else if (ageVal !== null && ageVal < 18 && creR.ok && creR.val !== null) {
    setAutoField('egfr', '18歳未満は適用外（日本人成人式）', '');
  } else {
  ...
  if (hbR.ok && hbR.val !== null) {
    const sex3 = vs('sex');
    if ((sex3 === 'male' && hbR.val < 13.0) || (sex3 === 'female' && hbR.val < 12.0)) {
      alerts.info.push({ title: '貧血（Hb低値）', msg: `Hb ${hbR.val} g/dL。貧血の併存によりHct法は欠乏量を過小評価する可能性があります。` });
    }
  }
  ...
  setSection('s5-status', s5Errors, s5Empty);
  ...
  const totalEmpty = s1Empty + s2Empty + s5Empty;
  ...
  // btnNextが有効になったらフッターを自動展開
  if (!btnNext.disabled && !footerOpen) {
    footerOpen = true;
    const actions = document.getElementById('footerActions');
    const toggleBtn = document.getElementById('btnFooterToggle');
...
//  SECTION STATUS
// ============================================================
function setSection(id, errors, empties = 0) {
  const el = document.getElementById(id);
  if (!el) return;
  if (errors > 0) { el.className = 'c-status error'; el.textContent = `エラー ${errors}件`; }
  else if (empties > 0) { el.className = 'c-status waiting'; el.textContent = `未入力 ${empties}件`; }
...
function resetAll() {
  ...
  // 現体重測定日は初期表示と同じく「今日」を復元する
  const _rNow = new Date();
  const _rPad = n => String(n).padStart(2, '0');
  document.getElementById('curWeightDate').value = _rNow.getFullYear() + '-' + _rPad(_rNow.getMonth()+1) + '-' + _rPad(_rNow.getDate());
...
function buildFluidPlan2(dtype, naVal, egfr, hfRisk, kVal, volFactor = 1.0) {
  ...
  const kHigh = kVal !== null && kVal > 5.5; // 高K血症 → eGFRに関わらずK含有製剤不可
  if (hfRisk) return {
  ...
  if (dtype === 'hypo') return {
    primary: 'ソルアセトF',
    secondary: (naVal !== null && naVal < 125) ? 'Na<125：3%NaCl投与は必ず医師の指示で（ODS予防）' : (kHigh ? 'ソルアセトF K 4mEq/L含有：K > 5.5 — 使用可否を医師に確認' : (kBad ? 'ソルアセトF K 4mEq/L含有：尿量・K値を観察' : 'Na<125・神経症状は医師へ即報告')),
    steps:['SBP < 90ならCriticalとして医師へ即報告してください。','循環維持が必要なら初期負荷（500mL/30分）を検討します。','Na < 125 や神経症状があれば必ず医師へ相談してください（ODS予防: ≤8〜10 mEq/day）。']
  };
  return {
    primary: 'ソルアセトF',
    secondary: kHigh ? 'K > 5.5 → K含有製剤（ソルデム3A等）使用不可。医師に確認' : (!kBad && !kCaut) ? 'ソルデム3A（K含有：eGFR>30のみ）' : 'K含有は医師に確認',
    steps:['急性期は500〜1000mLの初期輸液負荷を検討します。','維持期は100〜150 mL/hを参考に尿量と血圧反応を観察します。',kHigh ? 'K > 5.5 → 高カリウム血症のためソルデム3A等K含有輸液は使用不可です。2〜4時間ごとにK再検査を。' : kBad ? 'eGFR<30 → ソルデム3A等K含有輸液は禁忌です。' : kCaut ? 'eGFR<60 → K含有は慎重に。尿量・Kを2〜4時間ごとに確認。' : 'ソルデム3A（K含有）はeGFR>30の場合に使用可能です。']
  };
}
...
function goNext() {
  ...
  if (naChgH !== null) {
    // 上限: 急性（<48h相当）≤1.0 / 慢性（≥48h相当・不明時は安全側で慢性扱い）≤0.5 mEq/L/h
    if (naChgH <= 0.5) { naSafetyLabel = '安全範囲内（≤ 0.5 mEq/L/h）'; naSafetyClass = 'color:var(--success)'; }
    else if (naChgH <= 1.0 && !chronic) { naSafetyLabel = '要注意（急性のみ ≤ 1.0 mEq/L/h 許容）'; naSafetyClass = 'color:#E65100'; }
    else if (naChgH <= 1.0) { naSafetyLabel = '危険（慢性では 0.5 mEq/L/h 以下を厳守）'; naSafetyClass = 'color:var(--danger)'; }
  ...
  h += `</table><div style="margin-top:6px;font-size:11px;color:#666;">※ 成人27.5 mL/kg/日・高齢者22.5 mL/kg/日は、日本の運用基準を主軸としつつ、NICE CG174 を補助的に参照して本ツールで採用した参考値です。ただし、NICE CG174 は重症腎疾患（severe renal disease）を適用除外としています。小児 Holliday-Segar は文献整合的です。</div></div></div>`;
  ...
  h += '</table>';
  // 「尿量未評価」は入力の欠落通知であり輸液計画の判定根拠ではないため、表示上限5件の枠を臨床所見に譲る（画面上部のアラート欄には表示される）
  const p2Alerts = [...alerts.warning, ...alerts.info].filter(a => a.title !== '尿量未評価');
  if (p2Alerts.length > 0) {
    h += `<div class="p2-rec" style="margin-top:10px;"><strong>判定根拠（第1段階アラートより）</strong><br>`;
    p2Alerts.slice(0,5).forEach(a => { h += `• ${a.title}：${a.msg}<br>`; });
    h += '</div>';
  }
  h += '</div></div>';
...
const SAMPLE_CASES = [
  ...
  { label:'①高張性脱水・典型（80歳男性）', cat:'高張性脱水（Na>145）',
    ...
    sbp:96, dbp:58, hr:95, rr:22, spo2:95, temp:38.2, urineH:20,
...
function toggleFooter() {
  ...
  const actions = document.getElementById('footerActions');
  const btn = document.getElementById('btnFooterToggle');
  if (footerOpen) {
    actions.classList.remove('collapsed');
    btn.innerHTML = '▼ 閉じる';
  } else {
...
window.addEventListener('DOMContentLoaded', () => {
  ...
  setupFocusFlow();
  // フッター初期状態: 折りたたみ
  document.getElementById('footerActions').classList.add('collapsed');
  // 初期バリデーション
  validate();
});
